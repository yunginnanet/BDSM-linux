...
{
	private const  string  UserConfigFilename      = "UserConfiguration.yaml";
	...
	public static readonly SimpleUserConfiguration.Modpacks DefaultModpacksSimpleAIS =
		DefaultModpacksSimpleHS2 with { HS2Maps = false };
	...
	public static readonly RawUserConfiguration.Modpacks DefaultModpacksRawAIS =
		DefaultModpacksRawHS2 with { HS2Maps = false };

	public static void InitializeLogger(ILogger parent) => logger = parent;

	public static string ReadConfigAndDispose(string filename)
	{
		using StreamReader reader = File.OpenText(filename);
		return reader.ReadToEnd();
	}

	public static async Task<string> ReadConfigAndDisposeAsync(string filename)
	{
		using StreamReader reader = File.OpenText(filename);
	...
	public static async Task<FullUserConfiguration> GetOldUserConfigurationAsync()
	{
		...
		{
			old_config =
				yaml_deserializer.Deserialize<OldUserConfiguration>(
					await ReadConfigAndDisposeAsync(UserConfigFilename));
		}
		catch (Exception ex)
		{
			string ucex_message = ex switch
			{
				TypeInitializationException when ex.InnerException is FileNotFoundException =>
					"Your configuration file is missing. Please read the documentation and copy the example configuration to your own UserConfiguration.yaml.",
	...
	public static FullUserConfiguration GetUserConfiguration(out string config_version, string? config_yaml = null)
	{
		...
		try
		{
			raw_config = config_yaml is null
				? yaml_deserializer.Deserialize<RawUserConfiguration>(ReadConfigAndDispose(UserConfigFilename))
		...
		catch (Exception ex)
		{
			throw ex switch
			{
				...
				TypeInitializationException or YamlException => new UserConfigurationException(
					"Configuration file is malformed.", ex),
	...
	private static SimpleUserConfiguration ValidateRawUserConfiguration(RawUserConfiguration nullable_config,
		out string                                                                           config_version)
	{
		...
		bool isHs2;
		isHs2 = GamePathIsHS2(nullable_config.GamePath);
		if (!isHs2 && !GamePathIsAIS(nullable_config.GamePath))
			throw new UserConfigurationException(
				$"{nullable_config.GamePath} is not a valid HS2 or AIS game directory.");
		...
		return new SimpleUserConfiguration
		{
			GamePath = nullable_config.GamePath,
			OptionalModpacks = new SimpleUserConfiguration.Modpacks
			{
				Main             = main,
				MEShaders        = mes,
				...
				HS2Maps          = nullable_config.OptionalModpacks?.HS2Maps    ?? isHs2,
				BleedingEdge =
					nullable_config.OptionalModpacks?.BleedingEdge ?? DefaultModpacksSimpleHS2.BleedingEdge,
	...
	public static void SerializeUserConfiguration(SimpleUserConfiguration userconfig)
	{
		...
		using StreamWriter config_file_writer = File.CreateText(UserConfigFilename);
	...
	public static bool GamePathIsHS2(string path)
	{
		...
		foreach (FileInfo fileinfo in gamedir.EnumerateFiles())
			...
			{
				case "HoneySelect2.exe":
					return true;
				case "AI-Shoujo.exe" or "AI-Syoujyo.exe":
					return false;
			}
	...
	public static bool GamePathIsAIS(string path)
	{
		DirectoryInfo gamedir = new(path);
		if (!gamedir.Exists)
			return false;
	...
	public static SimpleUserConfiguration FullUserConfigurationToSimple(FullUserConfiguration full_config)
	{
			...
			switch (pm.FileName)
			{
				...
				default:
					throw new ArgumentException(
						$"The user configuration provided has invalid data. {pm.FileName} is not a valid modpack directory.");
			}
		...
		return new SimpleUserConfiguration
		{
			GamePath         = full_config.GamePath,
			PromptToContinue = full_config.PromptToContinue,
			OptionalModpacks = modpack_config
		};
	}
	...
	public static FullUserConfiguration SimpleUserConfigurationToFull(SimpleUserConfiguration simple_config)
	{
		...
		is_hs2 = _is_hs2 is not null
			? (bool)_is_hs2
			: throw new UserConfigurationException(
				$"{simple_config.GamePath} is not a valid HS2 or AIS game directory.");
		ImmutableHashSet<string> modpack_names = GetDesiredModpackNames(is_hs2, desired_modpacks);
		return new FullUserConfiguration
		{
			GamePath       = simple_config.GamePath,
			ConnectionInfo = DefaultConnectionInfo,
			BasePathMappings =
				ModpackNamesToPathMappings(modpack_names, simple_config.GamePath, DefaultConnectionInfo.RootPath),
	...
	public static PathMapping
		ModpackDefinitionToPathMapping(ModpackDefinition definition, string gamepath, string rootpath) => new()
	{
		RootPath           = rootpath,
